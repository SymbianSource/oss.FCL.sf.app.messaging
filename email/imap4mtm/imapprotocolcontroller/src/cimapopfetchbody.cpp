	...
	if (complete || ((aDoBodyText || related) && bodyTextComplete))
		...
		if (!entry.Complete())
			...
			if (complete || ((iServerEntry.Entry().iType==KUidMsvFolderEntry) && aPartialFetched))
				{
				if((iServerEntry.Entry().iType==KUidMsvMessageEntry||iServerEntry.Entry().iType==KUidMsvEmailTextEntry || iServerEntry.Entry().iType==KUidMsvEmailHtmlEntry ) && aPartialFetched)
					{
			...
			if(aPartialFetched)
				...
				if(entry.iType == KUidMsvAttachmentEntry)
				    entry.SetComplete(ETrue);
				
				
				}
			if((entry.Complete()) && (iServerEntry.Entry().iType == KUidMsvEmailTextEntry) || (iServerEntry.Entry().iType == KUidMsvEmailHtmlEntry ))
			    {
			    entry.SetBodyTextComplete(ETrue);
                entry.SetPartialDownloaded(EFalse);
			    }
            ...
            if (complete && (iServerEntry.Entry().iType==KUidMsvMessageEntry))
                {
                entry.SetComplete(ETrue);
                }
